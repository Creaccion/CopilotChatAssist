...
local function validate_java_structure(structure)
  ...
  if structure.class_declaration then
    ...
    for _, javadoc in ipairs(structure.javadocs) do
      -- CORRECCIÓN: Verificamos de manera más precisa si el JavaDoc está asociado a la clase
      -- Si el JavaDoc está justo antes de una anotación que está justo antes de la clase,
      -- o directamente antes de la clase, lo consideramos como JavaDoc de clase
      if javadoc.associated then
        -- Caso 1: Asociado directamente a la clase
        if javadoc.associated.type == "class" and
           javadoc.associated.line == structure.class_declaration.line then
          class_javadoc_found = true
        end

        -- Caso 2: Asociado a una anotación que está antes de la clase
        if javadoc.associated.type == "annotation" and annotation_before_class and
           javadoc.associated.line == nearest_annotation_line then
          class_javadoc_found = true
        end

        -- Verificar si está correctamente posicionado
        if class_javadoc_found then
